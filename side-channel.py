...
for k in range(255) :
    for i in range(2000) :
        cto_xor[i][k] = cto1[i] ^ k
        cto_xor[i][k] = SB(cto_xor[i][k])
        cto_xor[i][k] = H_distance(cto_xor[i][k], cto1[i])
        
#------------------------------------------------
ctox0 = np.transpose(cto_xor)

result = np.zeros((7200, 8000), dtype=float)

for i in range(7200, 8000):
    for k in range(255):
        result[k][i] = CorrelationPearson(ctox0[k], traces_transpose[i])

#------------------------------------------------
keys = np.zeros(255, dtype=float)

for k in range(255):
    keys[k] = np.max(result[k])

print(hex(np.argmax(keys)))
